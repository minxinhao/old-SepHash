...
constexpr uint64_t SLOT_PER_SEG = ((SEGMENT_SIZE - 4 * sizeof(uint64_t)) / (2 * sizeof(uint64_t)));
...
struct Slot
{
    ...
    void print()
    {
        ...
        printf("hash_value:%lu\t", hash_value);
        printf("size:%ld\n", sizeof(Slot));
    }
...
struct Directory
{
    ...
    void print()
    {
        ...
        {
            log_err("Entry %lu : local_depth:%lu cur_seg_ptr:%lx main_seg_ptr:%lx main_seg_lne:%lx", i,
                    segs[i].local_depth, segs[i].cur_seg_ptr, segs[i].main_seg_ptr, segs[i].main_seg_len);
        }
    }
} __attribute__((aligned(8)));
