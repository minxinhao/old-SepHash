...
Retry:
    ...
    auto read_slots = conn->read(seg_slots_ptr, seg_rmr.rkey, seg_slots, sizeof(Slot) * slots_len, lmr->lkey);
    ...
    co_await conn->write(slot_ptr + sizeof(uint64_t), seg_rmr.rkey,
...
task<> Client::Split(uint64_t seg_loc, uintptr_t seg_ptr, CurSegMeta *old_seg_meta)
{
    ...
    cur_seg->seg_meta.sign = !cur_seg->seg_meta.sign;
    co_await conn->write(seg_ptr + sizeof(uint64_t), seg_rmr.rkey, ((uint64_t *)cur_seg) + 1, sizeof(uint64_t),lmr->lkey);
}
